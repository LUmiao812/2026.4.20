    ...
    def optimize_and_evaluate(self, X_train, X_test, y_train, y_test, train_weights, class_weights):
        ...
        models = {
            ...
            "GBDT": GradientBoostingClassifier(
                loss='log_loss',
                n_estimators=300,       # 更多树 → 使用更多特征
                learning_rate=0.05,     # 小步学习 → 更细粒度
                max_depth=2,            # 更浅 → 不依赖强特征
                min_samples_split=2,    # 更容易分裂
                min_samples_leaf=1,     # 更容易分裂
                max_features='sqrt',   # 每次随机选特征 → 覆盖更多波长
                random_state=42,
                verbose=0
            )
        }
        ...
        if self.importance_records:
            importance_path = os.path.join(self.output_dir, "feature_importance_summary5.2.xlsx")
...
if __name__ == "__main__":
    # 光谱数据文件路径
    excel_path = r"D:\APP\Python\project\O2- protect model\MSC.SNV.D1400-1000.xlsx"
    # 更新权重文件路径为O2-浓度.xlsx
    weight_path = r"G:\10.11-10.15ZD 15 013579\O2-产生速率.xlsx"

    sheets = { "MSC": "MSC", "SNV": "SNV", "DERIV": "D1"}
    ...
    # ------------------- 汇总所有结果 -------------------
    summary_dir = "5.19=400-1000 0H TZspectral_model_results_with_weights_summary"
    os.makedirs(summary_dir, exist_ok=True)

    # 保存性能汇总
    df_summary = pd.DataFrame(all_results)
    summary_excel = os.path.join(summary_dir, "5-17 400-1000overall_performance_summary.xlsx")
    df_summary.to_excel(summary_excel, index=False)

    # 保存特征重要性汇总
    if all_importance_results:
        df_importance = pd.DataFrame(all_importance_results)
        importance_excel = os.path.join(summary_dir, "5-17 400-1000feature_importance_summary.xlsx")
